...
W=Path('/home/jimyao/.meridian/projects/orange-juniper-leaf/spawns/p34/work/data-generation')
balance=json.loads((W/'wave1-eval-balance.json').read_text())
report='''# Wave 1 evaluation task samples
...
```bash
PYTHONPATH=src uv run python3 - <<'PYVALIDATE'
import json, glob
...
The command was run with heredoc delimiter `PY` (the delimiter label above is expanded for readability only). Exit code 0; no stdout or stderr.
...
The actual heredoc delimiter was `PY`. Exit code 0; actual output:
...
'''
# Preserve the exact executed heredoc delimiters in the final report.
report=report.replace("<<'PYVALIDATE'","<<'PY'").replace('\nPYVALIDATE\n','\nPY\n')
report=report.replace('The command was run with heredoc delimiter `PY` (the delimiter label above is expanded for readability only). ', '')
report=report.replace('The actual heredoc delimiter was `PY`. ', '')
(W/'wave1-eval.md').write_text(report)
print('Wrote wave1-eval.md with target/actual balance and exact validation commands/results')
